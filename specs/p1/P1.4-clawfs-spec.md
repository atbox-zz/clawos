...
**Status:** 🔒 **FROZEN** (SHA256: `PLACEHOLDER_SHA256_TO_BE_FILLED`)
**Version:** 1.0.0
**Date:** 2026-02-24
...
#### `/clawfs/system/` — OS-Managed Files
...
/clawfs/system/
├── kernel/
│   ├── config-6.6-clawos          # Kernel .config (SHA256-signed)
│   └── modules/
│       ├── ebpf-monitor.ko        # eBPF monitoring module
│       └── clawfs-bridge.ko       # ClawFS kernel bridge
├── seccomp/
│   └── whitelist.json             # seccomp syscall whitelist (P1.2)
├── apparmor/
│   └── clawos-profile             # AppArmor profile (P1.6)
└── cgroup/
    └── quotas.json                # cgroup v2 resource quotas (P1.5)
```
...
#### `/clawfs/agents/` — Agent Workspaces
...
/clawfs/agents/
├── kernel-engine/
│   ├── workspace.db               # SQLite workspace (FTS5 + HNSW)
│   ├── state.json                 # Agent state snapshot
│   ├── logs/
│   │   └── agent-YYYY-MM-DD.log   # Daily logs (RULE-007)
...
#### `/clawfs/tools/` — Tool Definitions and WASM Binaries
...
```
/clawfs/tools/
├── registry.json                  # Tool manifest (SHA256-signed)
├── definitions/
...
#### `/clawfs/workspaces/` — User Workspaces
...
#### `/clawfs/vault/` — Encrypted Secrets Storage
...
```
/clawfs/vault/
├── master-key.enc                # Encrypted master key (AES-256-GCM)
├── secrets/
│   ├── openai-api-key.enc
│   ├── near-ai-token.enc
│   ├── github-pat.enc
│   └── ...
├── system-manifest.json          # SHA256 hashes of system files
└── spec-signatures.json          # SHA256 signatures of P1 specs
```
...
#### `/clawfs/specs/` — Frozen Specification Documents
...
/clawfs/specs/
├── p1/
│   ├── P1.1-wit-interface-spec.md
│   ├── P1.2-seccomp-whitelist-spec.md
│   ├── P1.3-ebpf-event-struct-spec.md
│   ├── P1.4-clawfs-spec.md        # This document
│   ├── P1.5-cgroup-quotas-spec.md
│   ├── P1.6-apparmor-profile-spec.md
│   ├── P1.7-ipc-protocol-spec.md
│   └── P1.8-public-api-spec.md
└── signatures.json                # SHA256 signatures of all P1 specs
```
...
| **Max length** | Path component ≤ 255 characters | Enforced by filesystem |
...
```
keyring: .clawos
  ├── key: clawos-master (type: user, 256-bit)
  ├── key: clawos-tpm-sealed (type: trusted, if TPM available)
  └── key: clawos-derived-{secret-id} (type: logon, per-secret)
```
...
5. Store: `{iv || ciphertext || auth_tag}` (concatenated)
...
fn encrypt_secret(plaintext: &[u8], key: &[u8; 32]) -> Result<Vec<u8>, Error> {
    ...
    // Return: IV (12) || Ciphertext (variable) || Tag (16)
...
- [ ] Key derivation hierarchy is clear (TPM → Keyring → Per-secret)
...
**Status:** 🔒 **FROZEN** (SHA256: `PLACEHOLDER_SHA256_TO_BE_FILLED`)
**Next Phase:** P2.7 — ClawFS Rust crate skeleton implementation
