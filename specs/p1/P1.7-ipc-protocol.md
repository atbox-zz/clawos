...
|--------|-----------|-------------|
| `execute_tool` | Agent Loop → WASM Daemon | Execute a tool in WASM runtime |
| `get_metrics` | Any → Observability | Query current metrics |
| `check_policy` | Agent Loop → Security Agent | Verify action against policy |
| `read_file` | Agent Loop → FS Engine | Read file from ClawFS |
| `write_file` | Agent Loop → FS Engine | Write file to ClawFS |
| `schedule_task` | Agent Loop → Scheduler | Schedule a task |
| `route_message` | Router → Any | Route message to destination |
| `subscribe_events` | Any → eBPF Agent | Subscribe to eBPF events |
...
**Direction:** Any → Any (typically unidirectional)
...
| 1.0 | ✅ 1.0 | ✅ 1.0 | ❌ Reject |
| 1.1 | ✅ 1.0 | ✅ 1.1 | ❌ Reject |
| 2.0 | ❌ Reject | ❌ Reject | ✅ 2.0 |

**Version Evolution Rules:**
1. **Minor version bump (1.0 → 1.1):** Additive changes only (new fields, new methods)
2. **Major version bump (1.x → 2.0):** Breaking changes (removed fields, changed semantics)
