...
- **Latency**: ~100μs (local file), ~10ms (encrypted file)
...
- **Blocking**: Yes (synchronous I/O)
- **Timeout**: 5 seconds (configurable via cgroup)
- **Latency**: ~50μs (local file), ~5ms (encrypted file)
- **Throughput**: Up to 1 GB/s (local SSD)
...
- **Blocking**: Yes (synchronous I/O)
- **Timeout**: 5 seconds (configurable via cgroup)
- **Latency**: ~50μs (local file), ~10ms (encrypted file)
- **Throughput**: Up to 1 GB/s (local SSD)
...
- **Timeout**: None
- **Latency**: ~10μs (cached), ~1ms (uncached)
...
- **Latency**: ~10μs (WNOHANG), variable (blocking)
...
- **Blocking**: No (immediate)
- **Timeout**: None
- **Latency**: ~1μs (small allocation), ~100μs (large allocation)
...
- **Blocking**: No (immediate)
- **Timeout**: None
- **Latency**: ~100μs (subscription setup)
- **Event Delivery**: ~10μs per event
...
### Error Code Mapping (WIT → Syscall)
...
|----------|---------|------------|----------|---------|---------|
| File System | `claw_fs_open` | `CAP_CLAWFS_READ/WRITE` | Yes | 5s | ~100μs |
| File System | `claw_fs_read` | `CAP_CLAWFS_READ` | Yes | 5s | ~50μs |
| File System | `claw_fs_write` | `CAP_CLAWFS_WRITE` | Yes | 5s | ~50μs |
| File System | `claw_fs_close` | None | No | None | ~10μs |
| File System | `claw_fs_stat` | `CAP_CLAWFS_READ` | No | None | ~10μs |
| Network | `claw_net_connect` | `CAP_CLAWNET_CONNECT` | Yes | 10s | ~1ms |
| Network | `claw_net_send` | `CAP_CLAWNET_SEND` | Yes | 5s | ~10μs |
| Network | `claw_net_recv` | `CAP_CLAWNET_RECV` | Yes | 5s | ~10μs |
| Network | `claw_net_close` | None | No | None | ~10μs |
| Process | `claw_proc_spawn` | `CAP_CLAWPROC_SPAWN` | Yes | 30s | ~10ms |
| Process | `claw_proc_wait` | `CAP_CLAWPROC_WAIT` | Yes* | None | ~10μs |
| Process | `claw_proc_kill` | `CAP_CLAWPROC_KILL` | No | None | ~10μs |
| Memory | `claw_mem_alloc` | `CAP_CLAWMEM_ALLOC` | No | None | ~1μs |
| Memory | `claw_mem_free` | `CAP_CLAWMEM_FREE` | No | None | ~1μs |
| Security | `claw_seccomp_apply` | `CAP_CLAWSEC_SECCOMP` | No | None | ~100μs |
| Security | `claw_cgroup_set` | `CAP_CLAWSEC_CGROUP` | No | None | ~100μs |
| Security | `claw_namespace_enter` | `CAP_CLAWSEC_NAMESPACE` | No | None | ~10μs |
| Monitoring | `claw_metric_query` | `CAP_CLAWMON_METRIC` | No | None | ~10μs |
| Monitoring | `claw_log_write` | `CAP_CLAWMON_LOG` | No | None | ~10μs |
| Monitoring | `claw_event_subscribe` | `CAP_CLAWMON_EVENT` | No | None | ~100μs |
...
- **P1.1:** WIT Interface Specification Book (WASM ↔ Kernel ABI)
