...
echo "=== Violations from journalctl ===" | tee "$VIOLATION_LOG"
sudo journalctl -u apparmor --since "7 days ago" | grep -E "(DENIED|REJECTING)" | tee -a "$VIOLATION_LOG"

# Collect violations from audit log
echo -e "\n=== Violations from audit log ===" | tee -a "$VIOLATION_LOG"
sudo ausearch -m AVC -ts recent | grep apparmor | tee -a "$VIOLATION_LOG"

# Collect violations from dmesg
echo -e "\n=== Violations from dmesg ===" | tee -a "$VIOLATION_LOG"
sudo dmesg | grep -i apparmor | grep -E "(DENIED|REJECTING)" | tee -a "$VIOLATION_LOG"

# Generate summary
echo -e "\n=== Violation Summary ===" | tee -a "$VIOLATION_LOG"
echo "Total violations: $(sudo journalctl -u apparmor --since "7 days ago" | grep -c -E "(DENIED|REJECTING)")" | tee -a "$VIOLATION_LOG"
...
for profile in "${PROFILES[@]}"; do
    ...
    grep -E "deny.*(/etc/passwd|/etc/shadow|/etc/sudoers|/etc/ssh)" "$profile" | while read -r line; do
        echo "  [KEEP] $line"
    done
...
grep -E "(DENIED|REJECTING).*config" "$VIOLATION_LOG" | while read -r line; do
    echo "  $line"
...
grep -E "(DENIED|REJECTING).*\.so" "$VIOLATION_LOG" | while read -r line; do
    echo "  $line"
...
grep -E "(DENIED|REJECTING).*(/tmp|/var/tmp)" "$VIOLATION_LOG" | while read -r line; do
    echo "  $line"
...
grep -E "(DENIED|REJECTING).*log" "$VIOLATION_LOG" | while read -r line; do
    echo "  $line"
...
grep -E "(DENIED|REJECTING).*(/run|/var/run)" "$VIOLATION_LOG" | while read -r line; do
    echo "  $line"
...
case "$PROFILE" in
    clawos-agent-loop)
        case "$PATH" in
            /etc/clawos/*|/var/lib/clawos/*|/var/log/clawos/*|/run/clawos/*)
    ...
    clawos-wasm-daemon)
        case "$PATH" in
            /var/lib/clawos/wasm-sandbox/*|/var/lib/clawos/wasm-cache/*|/etc/clawos/wasm/*)
    ...
    clawos-kernel-svc)
        case "$PATH" in
            /sys/fs/bpf/*|/sys/fs/cgroup/*|/etc/clawos/kernel/*)
...
sudo journalctl -u apparmor --since "1 day ago" | grep -E "ALLOWED" | grep -v "expected_pattern"

# Verify deny rules are working
echo "Verifying deny rules..."
sudo journalctl -u apparmor --since "1 day ago" | grep -E "DENIED" | wc -l

# Check performance impact
echo "Checking performance impact..."
sudo systemctl status clawos-agent-loop | grep -E "Active|Memory"
...
sudo journalctl -u apparmor -f | while read -r line; do
    if echo "$line" | grep -qE "(DENIED|REJECTING)"; then
        # Log the violation
        ...
        # Update summary
        {
            echo "$(date -Iseconds) | $PROFILE | $OPERATION | $NAME"
        } >> "$SUMMARY_LOG"

        # Alert if critical violation
        if echo "$line" | grep -qE "(passwd|shadow|sudoers)"; then
...
```
2026-02-24T10:30:45+00:00 | clawos-agent-loop | open | /etc/clawos/config.json | requested_mask="r" | denied_mask="r" | pid=1234
```
...
echo -e "\n=== Violations by Profile ==="
awk -F'|' '{print $3}' "$VIOLATION_LOG" | sort | uniq -c | sort -rn

# Violations by operation
echo -e "\n=== Violations by Operation ==="
awk -F'|' '{print $4}' "$VIOLATION_LOG" | sort | uniq -c | sort -rn

# Top 10 most violated paths
echo -e "\n=== Top 10 Most Violated Paths ==="
awk -F'|' '{print $5}' "$VIOLATION_LOG" | sort | uniq -c | sort -rn | head -10

# Security-sensitive violations
echo -e "\n=== Security-Sensitive Violations ==="
grep -E "(passwd|shadow|sudoers|ssh)" "$VIOLATION_LOG"
...
CRITICAL_VIOLATIONS=$(sudo journalctl -u apparmor --since "1 hour ago" | grep -E "(DENIED|REJECTING)" | grep -E "(passwd|shadow|sudoers|ssh)" | wc -l)
...
VIOLATION_RATE=$(sudo journalctl -u apparmor --since "1 hour ago" | grep -cE "(DENIED|REJECTING)")
...
if [ -z "$PROFILE" ] || [ -z "$CHANGE_DESCRIPTION" ] || [ -z "$JUSTIFICATION" ]; then
...
if jq -r '.change_description' "$REQUEST_FILE" | grep -qiE "(deny|capability|network|passwd|shadow)"; then
...
if [ -z "$REQUEST_ID" ] || [ -z "$DECISION" ]; then
    echo "Usage: $0 <request_id> <approve|reject|conditional> [comments]"
    exit 1
fi
...
awk -F'|' '{print $3 "|" $4 "|" $5}' "$VIOLATION_LOG" | sort -u > "$UNIQUE_VIOLATIONS"
...
grep -E "(/run|/var/run)" "$UNIQUE_VIOLATIONS" > "$RUNTIME_VIOLATIONS" 2>/dev/null
grep -E "(passwd|shadow|sudoers|ssh)" "$UNIQUE_VIOLATIONS" > "$SECURITY_VIOLATIONS" 2>/dev/null
...
if [ -z "$VIOLATION_LOG" ] || [ -z "$PROFILE" ]; then
...
# Generate rules from violations
{
    ...
    awk -F'|' '{
        ...
        if (profile == "'"$PROFILE"'") {
            # Generate rule based on operation type
            if (operation == "open" || operation == "create" || operation == "unlink") {
...
VIOLATIONS=$(sudo journalctl -u apparmor --since "5 minutes ago" | grep -cE "(DENIED|REJECTING).*$PROFILE")
