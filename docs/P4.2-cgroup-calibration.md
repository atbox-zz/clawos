...
for i in $(seq 1 $((CALIBRATION_DURATION / 5))); do
  ...
  memory_swap_current=$(cat $CGROUP_PATH/memory.swap.current 2>/dev/null || echo "0")
...
for i in $(seq 1 $((CALIBRATION_DURATION / 5))); do
  ...
  pressure_full=$(cat $CGROUP_PATH/memory.pressure 2>/dev/null || echo "")

  some_avg60=$(echo "$pressure_some" | grep "some avg60=" | cut -d'=' -f2 | cut -d' ' -f1)
  some_avg300=$(echo "$pressure_some" | grep "some avg300=" | cut -d'=' -f2 | cut -d' ' -f1)
  some_avg600=$(echo "$pressure_some" | grep "some avg600=" | cut -d'=' -f2 | cut -d' ' -f1)

  full_avg60=$(echo "$pressure_full" | grep "full avg60=" | cut -d'=' -f2 | cut -d' ' -f1)
  full_avg300=$(echo "$pressure_full" | grep "full avg300=" | cut -d'=' -f2 | cut -d' ' -f1)
  full_avg600=$(echo "$pressure_full" | grep "full avg600=" | cut -d'=' -f2 | cut -d' ' -f1)
...
for i in $(seq 1 $((CALIBRATION_DURATION / 5))); do
  ...
  usage_usec=$(echo "$cpu_stat" | grep "usage_usec" | cut -d' ' -f2)
  user_usec=$(echo "$cpu_stat" | grep "user_usec" | cut -d' ' -f2)
  system_usec=$(echo "$cpu_stat" | grep "system_usec" | cut -d' ' -f2)
  nr_periods=$(echo "$cpu_stat" | grep "nr_periods" | cut -d' ' -f2)
  nr_throttled=$(echo "$cpu_stat" | grep "nr_throttled" | cut -d' ' -f2)
  throttled_usec=$(echo "$cpu_stat" | grep "throttled_usec" | cut -d' ' -f2)
...
for i in $(seq 1 $((CALIBRATION_DURATION / 5))); do
  ...
  nr_latencies=$(echo "$cpu_stat" | grep "nr_latencies" | cut -d' ' -f2)
  latency_avg_usec=$(echo "$cpu_stat" | grep "latency_avg_usec" | cut -d' ' -f2)
  latency_max_usec=$(echo "$cpu_stat" | grep "latency_max_usec" | cut -d' ' -f2)
...
for i in $(seq 1 $((CALIBRATION_DURATION / 5))); do
  ...
  rbytes=$(echo "$io_stat" | grep "rbytes=" | cut -d'=' -f2 | cut -d' ' -f1)
  wbytes=$(echo "$io_stat" | grep "wbytes=" | cut -d'=' -f2 | cut -d' ' -f1)
  rios=$(echo "$io_stat" | grep "rios=" | cut -d'=' -f2 | cut -d' ' -f1)
  wios=$(echo "$io_stat" | grep "wios=" | cut -d'=' -f2 | cut -d' ' -f1)
  dbytes=$(echo "$io_stat" | grep "dbytes=" | cut -d'=' -f2 | cut -d' ' -f1)
  dios=$(echo "$io_stat" | grep "dios=" | cut -d'=' -f2 | cut -d' ' -f1)
...
while [ $iteration -lt $max_iterations ]; do
  ...
  pressure_some=$(awk -F',' 'NR>1 {sum+=$2; count++} END {print sum/count}' \
    /tmp/cgroup-calibration/raw/memory_pressure_${COMPONENT}_${iteration}.csv 2>/dev/null || echo "0")
...
while [ $iteration -lt $max_iterations ]; do
  ...
  first_rbytes=$(head -2 /tmp/cgroup-calibration/raw/io_${COMPONENT}_${iteration}.csv | tail -1 | cut -d',' -f2)
  last_rbytes=$(tail -1 /tmp/cgroup-calibration/raw/io_${COMPONENT}_${iteration}.csv | cut -d',' -f2)
  first_wbytes=$(head -2 /tmp/cgroup-calibration/raw/io_${COMPONENT}_${iteration}.csv | tail -1 | cut -d',' -f3)
  last_wbytes=$(tail -1 /tmp/cgroup-calibration/raw/io_${COMPONENT}_${iteration}.csv | cut -d',' -f3)
...
# Phase 2: Tuning
echo "Phase 2: Tuning..."
CGROUP_PATH=$(jq -r ".components[] | select(.name == \"$COMPONENT\") | .cgroup_path" specs/p1/P1.5-cgroup-quotas.json)

MEMORY_LIMIT=$(jq -r ".components[] | select(.name == \"$COMPONENT\") | .resource_limits.memory.max" specs/p1/P1.5-cgroup-quotas.json)
CPU_QUOTA=$(jq -r ".components[] | select(.name == \"$COMPONENT\") | .resource_limits.cpu.max" specs/p1/P1.5-cgroup-quotas.json | cut -d' ' -f1)
PIDS_LIMIT=$(jq -r ".components[] | select(.name == \"$COMPONENT\") | .resource_limits.pids.max" specs/p1/P1.5-cgroup-quotas.json)
IO_WEIGHT=$(jq -r ".components[] | select(.name == \"$COMPONENT\") | .resource_limits.io.weight" specs/p1/P1.5-cgroup-quotas.json)
