# P3.8 — Secrets Encryption Key Initialization Specification
...
│   └── Purpose: Root master key for all secret encryption
├── Key: clawos-tpm-sealed (type: trusted, if TPM available)
│   └── Purpose: TPM-sealed master key blob
├── Key: clawos-derived-{secret-id} (type: logon, per-secret)
│   └── Purpose: Derived keys for individual secrets
└── Key: clawos-rotation-{timestamp} (type: user, during rotation)
    └── Purpose: Temporary key during key rotation
```
...
| fixedparent | sensitivedataorigin | userwithauth | noda |
...
```
Master Key (256-bit)
├── Derived Key 1: openai-api-key (256-bit)
│   └── Salt: "openai-api-key:v1"
├── Derived Key 2: near-ai-token (256-bit)
│   └── Salt: "near-ai-token:v1"
├── Derived Key 3: github-pat (256-bit)
│   └── Salt: "github-pat:v1"
└── ...
```
...
├── master-key.salt                   # Salt for software fallback
├── master-key.tpm                    # TPM-sealed key blob (if TPM available)
├── secrets/                          # Encrypted secrets
│   ├── openai-api-key.enc
│   ├── near-ai-token.enc
│   ├── github-pat.enc
│   └── ...
├── archive/                          # Archived keys during rotation
│   └── 20240224/
│       └── master-key.enc.old
├── recovery/                         # Recovery keys
│   └── recovery-key.enc              # Emergency recovery key
