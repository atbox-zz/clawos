...
| select(.verified_by_strace == true or .tokio_required == true) |
...
| select(.name as $n | $remove | index($n) |
...
| length' specs/p1/P1.2-seccomp-whitelist.backup.*.json |
|  |
