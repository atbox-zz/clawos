...
1. Validate endpoint against allowlist
   ├─ Not found → ENOENT (3) → Return 404
   └─ Found → Continue

2. Check rate limits
   ├─ Exceeded → EAGAIN (1) → Return 429 with Retry-After
   └─ OK → Continue

3. Validate security policies
   ├─ TLS failed → EPERM (4) → Log security event
   ├─ Signature invalid → EPERM (4) → Log security event
   └─ OK → Continue

4. Make request to provider
   ├─ Timeout → ETIMEOUT (6) → Retry up to 2 times
   ├─ Network error → EIO (2) → Retry up to 3 times
