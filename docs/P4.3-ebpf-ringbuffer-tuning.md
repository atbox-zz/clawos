...
```
BUFFER_SIZE = (EVENT_SIZE × EVENTS_PER_SEC × BUFFER_TIME) × SAFETY_FACTOR
```
...
BUFFER_SIZE = 104 × 10,000 × 2 × 3
           = 6,240,000 bytes
           ≈ 6 MB (rounded to power of 2)
```
...
BUFFER_SIZE = 1056 × 100 × 3 × 2
           = 633,600 bytes
           ≈ 1 MB (rounded to power of 2)
```
...
BUFFER_SIZE = 72 × 20,000 × 1 × 4
           = 5,760,000 bytes
           ≈ 8 MB (rounded to power of 2)
```
...
**Formula:**
```
BENEFIT = (EVENTS_SAVED × EVENT_VALUE) - (MEMORY_COST × MEMORY_PRICE)
```
...
BENEFIT = (1,000 × $10,000) - (0.006 GB × $0.01)
        = $10,000,000 - $0.00006
        = $9,999,999.94
...
| ---------| ----------| ------------| --------------| --------------|
| Normal   | 100/sec   | 3 sec       | 2x            | 640 KB → 1 MB |
| Attack   | 1,000/sec | 5 sec       | 4x            | 21 MB → 32 MB |
| Forensic | 100/sec   | 10 sec      | 2x            | 2 MB → 2 MB   |
...
| Scenario | Rate       | Buffer Time | Safety Factor | Buffer Size     |
| ---------| -----------| ------------| --------------| ----------------|
| Idle     | 1,000/sec  | 2 sec       | 2x            | 416 KB → 512 KB |
| Normal   | 10,000/sec | 2 sec       | 3x            | 6.2 MB → 8 MB   |
| Peak     | 50,000/sec | 1 sec       | 4x            | 20.8 MB → 32 MB |
...
| Scenario | Rate       | Buffer Time | Safety Factor | Buffer Size     |
| ---------| -----------| ------------| --------------| ----------------|
| Normal   | 5,000/sec  | 2 sec       | 2x            | 6 MB → 8 MB     |
| Scan     | 50,000/sec | 3 sec       | 3x            | 136 MB → 256 MB |
| Backup   | 20,000/sec | 2 sec       | 2x            | 24 MB → 32 MB   |
...
| Scenario | Rate          | Buffer Time | Safety Factor | Buffer Size     |
| ---------| --------------| ------------| --------------| ----------------|
| Idle     | 1,000/sec     | 1 sec       | 2x            | 144 KB → 256 KB |
| Normal   | 20,000/sec    | 1 sec       | 3x            | 4.3 MB → 8 MB   |
| DDoS     | 1,000,000/sec | 0.5 sec     | 5x            | 180 MB → 256 MB |
...
| Scenario | Rate       | Buffer Time | Safety Factor | Buffer Size    |
| ---------| -----------| ------------| --------------| ---------------|
| Normal   | 500/sec    | 3 sec       | 2x            | 912 KB → 1 MB  |
| Scaling  | 5,000/sec  | 2 sec       | 3x            | 9.1 MB → 16 MB |
| Stress   | 10,000/sec | 1 sec       | 4x            | 12 MB → 16 MB  |
...
BUFFER_SIZE = round_to_power_of_2(
    EVENT_SIZE × EVENTS_PER_SEC × BUFFER_TIME × SAFETY_FACTOR
)
```
