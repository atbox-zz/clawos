...
ROLE_MANIPULATION_PATTERNS = [
    ...
    r'pretend\s+(you\s+)?(are|were)\s+(a|an)\s+[^.!?]+',
    r'roleplay\s+as\s+(a|an)\s+[^.!?]+',
    r'(?i)^(you\s+are|act\s+as|pretend)\s+',
]

# Context Extraction Patterns
CONTEXT_EXTRACTION_PATTERNS = [
    r'repeat\s+(everything\s+)?(above|before)\s+(this\s+)?(line|point)',
    r'print\s+(your\s+)?(system\s+)?(prompt|instructions|configuration)',
    r'output\s+(your\s+)?(prompt|instructions)\s+in\s+(json|xml|yaml)',
    r'show\s+me\s+(your\s+)?(instructions|guidelines|rules)',
    r'tell\s+me\s+(exactly|word\s+for\s+word)\s+what\s+(you\s+)?(were\s+)?told',
]

# Encoding Attack Patterns
ENCODING_ATTACK_PATTERNS = [
    r'decode\s+(this\s+)?(base64|hex|rot13|binary)\s+(and\s+)?(execute|run|interpret)',
    r'interpret\s+(this\s+)?(base64|hex|rot13|binary)\s+(as\s+)?(code|command)',
    r'convert\s+(this\s+)?(base64|hex|rot13|binary)\s+to\s+(text|code)',
    r'(?i)(base64|hex|rot13|binary)\s*[:=]\s*[A-Za-z0-9+/=]+',
]

# Privilege Escalation Patterns
PRIVILEGE_ESCALATION_PATTERNS = [
    r'enable\s+(developer|admin|root|debug)\s+mode',
    r'switch\s+to\s+(admin|developer|root)\s+mode',
    r'unlock\s+(all\s+)?(features|permissions|capabilities)',
    r'bypass\s+(the\s+)?(safety|security|filter|restriction)',
    r'disable\s+(the\s+)?(safety|security|filter|restriction)',
]

# Code Injection Patterns
CODE_INJECTION_PATTERNS = [
    r'execute\s+(this\s+)?(python|javascript|bash|shell)\s+code',
