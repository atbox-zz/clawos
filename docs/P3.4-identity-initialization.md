# P3.4 — Identity Files and Workspace Initialization Specification
...
This specification defines the initialization and management of **Identity Files** and **Agent Workspaces** in ClawOS. Identity Files serve as the persistent kernel memory mechanism for AI agents, storing learned patterns, heuristics, and execution state across sessions.
...
```json
{
  ...
  "pattern_type": "syscall_dependency|code_pattern|workflow_pattern",
  "confidence": 0.0-1.0,
  ...
  "source": "observation|user_input|inference",
  "tags": ["tag1", "tag2"]
}
```
...
```json
{
  ...
  "rule_type": "resource_allocation|error_prediction|optimization",
  "accuracy": 0.0-1.0,
...
/clawfs/agents/
├── kernel-engine/
│   ├── identity.json                 # Agent identity file (Section 1)
│   ├── workspace.db                  # SQLite + HNSW database
│   ├── state.json                    # Runtime state snapshot
│   ├── config.json                   # Agent-specific configuration
│   ├── logs/
│   │   ├── agent-2024-02-24.log      # Daily logs (RULE-007)
│   │   ├── agent-2024-02-25.log
│   │   └── .gitkeep
...
│   │   ├── checkpoint-2024-02-24-100000.json
│   │   └── .gitkeep
│   ├── backups/                      # Identity file backups
│   │   ├── identity-2024-02-24-000000.json
│   │   └── .gitkeep
│   └── .gitkeep
├── ebpf-agent/
│   ├── identity.json
    ...
    cat > "${identity_file}" <<EOF
{
  ...
  "checksum": "sha256:$(echo '{}' | sha256sum | cut -d' ' -f1)"
}
EOF

    # Calculate and update checksum
    local checksum=$(cat "${identity_file}" | jq -c 'del(.checksum)' | sha256sum | cut -d' ' -f1)
...
impl IdentityInitializer {
    ...
    fn create_directory_structure(&self, agent_dir: &Path) -> Result<()> {
        ...
        for dir in &dirs {
            ...
            fs::write(&gitkeep, "")
                .with_context(|| format!("Failed to create .gitkeep: {:?}", gitkeep))?;
...
```
/clawfs/agents/{agent}/
├── identity.json              # Primary memory (current state)
├── workspace.db               # Vector-indexed knowledge base
├── checkpoints/               # State snapshots
│   ├── checkpoint-2024-02-24-100000.json
│   └── checkpoint-2024-02-24-110000.json
└── backups/                   # Identity file backups
    ├── identity-2024-02-24-000000.json
    ├── identity-2024-02-24-060000.json
    └── identity-2024-02-24-120000.json
```
...
└─────────────────────────────────────────────────────────────┘
                              ...
                              ▼
┌─────────────────────────────────────────────────────────────┐
│              Update Identity File (in-memory)                │
│  - Update state                                             │
...
│  - Hash entire JSON (excluding checksum field)              │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│              Create Backup (if changed)                     │
│  - Copy current identity.json to backups/                   │
...
┌─────────────────────────────────────────────────────────────┐
│              Atomic Write                                   │
│  - Write to temp file: identity.json.tmp                   │
│  - Verify checksum                                          │
│  - Rename: identity.json.tmp → identity.json               │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│              Create Checkpoint (if task completed)          │
│  - Snapshot state to checkpoints/                           │
│  - Timestamp: checkpoint-{timestamp}.json                   │
└─────────────────────────────────────────────────────────────┘
```
...
pub fn calculate_identity_checksum(identity: &Value) -> Result<String> {
    ...
    identity_without_checksum.as_object_mut()
        .ok_or_else(|| anyhow::anyhow!("Identity is not an object"))?
        .remove("checksum");
...
pub fn verify_identity_checksum(identity: &Value) -> Result<bool> {
    ...
    let stored_checksum = identity.get("checksum")
        .and_then(|c| c.as_str())
        .ok_or_else(|| anyhow::anyhow!("Checksum field missing"))?;
...
ACTUAL_HASH=$(jq -c 'del(.checksum)' "${IDENTITY_FILE}" | sha256sum | cut -d' ' -f1)

# Compare
if [[ "${STORED_HASH}" == "${ACTUAL_HASH}" ]]; then
    echo "✓ Checksum verified: ${IDENTITY_FILE}"
    echo "  SHA-256: ${ACTUAL_HASH}"
    exit 0
else
    echo "✗ Checksum mismatch: ${IDENTITY_FILE}"
...
┌─────────────────────────────────────────────────────────────┐
│              Extract Stored Checksum                        │
│  - Read checksum field from identity.json                   │
│  - Remove "sha256:" prefix                                  │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│              Calculate Actual Checksum                      │
│  - Remove checksum field from JSON                          │
│  - Serialize to string                                      │
│  - Calculate SHA-256                                        │
└─────────────────────────────────────────────────────────────┘
                              │
