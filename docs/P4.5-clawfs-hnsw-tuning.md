# P4.5 — ClawFS HNSW Parameter Tuning Specification
...
│    - Vector dimension (1536 or 3072)                        │
│    - Target recall (e.g., 0.95)                              │
│    - Max search latency (e.g., 100ms)                        │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│ 2. Baseline Benchmarking                                     │
│    - Test with default parameters (M=16, ef_construction=200)│
│    - Measure: build time, search latency, recall@10         │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│ 3. Parameter Grid Search                                     │
│    - M: [8, 12, 16, 24, 32, 48, 64]                         │
│    - ef_construction: [100, 150, 200, 300, 400, 600, 800]  │
│    - ef_search: [10, 25, 50, 100, 150, 200]                │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│ 4. Pareto Analysis                                           │
│    - Identify optimal trade-offs                             │
│    - Select parameters meeting requirements                  │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│ 5. Validation                                                │
...
| **Recall@10**      | Fraction of true top-10 found  | ≥ 0.95 for production    |
| **Recall@100**     | Fraction of true top-100 found | ≥ 0.98 for production    |
| **Memory Usage**   | Index size in RAM              | < 2× vector data size    |
| **Index Size**     | Disk footprint                 | < 1.5× vector data size  |
...
```
Build Time ∝ ef_construction × log(N)

Example (100K vectors, M=16):
- ef_construction=100:  ~45 seconds
- ef_construction=200:  ~90 seconds  (2×)
- ef_construction=400:  ~180 seconds (4×)
- ef_construction=800:  ~360 seconds (8×)
```
...
**Diminishing Returns:**
- 100 → 200: +5% recall, 2× build time
- 200 → 400: +3% recall, 2× build time
- 400 → 800: +1.5% recall, 2× build time
...
```
Memory ≈ dimension × 4 × M × N × (1 + 1/ln(M))
...
**Diminishing Returns:**
- 8 → 16: +6% recall, 1.8× memory
- 16 → 32: +3% recall, 1.7× memory
- 32 → 64: +1.5% recall, 1.8× memory
...
```
Search Latency ∝ ef_search × log(N)
...
**Diminishing Returns:**
- 10 → 50: +12% recall, 5× latency
- 50 → 100: +3% recall, 2× latency
- 100 → 200: +1.5% recall, 2× latency
...
| ---------------------| -----------| --------------| --------|
| Build Time           | 90 seconds | < 300 seconds | ✅ PASS  |
| Index Size           | 98 MB      | < 150 MB      | ✅ PASS  |
| Memory Usage         | 120 MB     | < 200 MB      | ✅ PASS  |
| Search Latency (P50) | 20 ms      | < 50 ms       | ✅ PASS  |
| Search Latency (P95) | 35 ms      | < 100 ms      | ✅ PASS  |
| Search Latency (P99) | 50 ms      | < 150 ms      | ✅ PASS  |
| Recall@10            | 0.94       | ≥ 0.95        | ⚠️ NEAR |
| Recall@100           | 0.97       | ≥ 0.98        | ⚠️ NEAR |
...
**Recommendation:** ef_construction=300 for recall ≥ 0.96, ef_construction=200 for balanced performance.
...
**Key Insights:**
- 100 → 200: +5% recall for +45s build time (GOOD)
- 200 → 300: +2% recall for +45s build time (ACCEPTABLE)
- 300 → 400: +1% recall for +45s build time (MARGINAL)
- 400 → 800: +1.5% recall for +180s build time (POOR)
...
**Key Insights:**
- 8 → 16: +6% recall for +50MB memory (GOOD)
- 16 → 24: +2% recall for +50MB memory (ACCEPTABLE)
- 24 → 32: +1% recall for +50MB memory (MARGINAL)
- 32 → 64: +1.5% recall for +180MB memory (POOR)
...
| ----------| ------------| ----------------| -------------|
| 100 → 150 | +3%         | +23s            | 0.13%/s      |
| 150 → 200 | +2%         | +22s            | 0.09%/s      |
| 200 → 300 | +2%         | +45s            | 0.04%/s      |
| 300 → 400 | +1%         | +45s            | 0.02%/s      |
| 400 → 800 | +1.5%       | +180s           | 0.008%/s     |
...
| --------| ------------| ------------| -------------|
| 8 → 12  | +3%         | +25MB       | 0.12%/MB     |
| 12 → 16 | +3%         | +25MB       | 0.12%/MB     |
| 16 → 24 | +2%         | +50MB       | 0.04%/MB     |
| 24 → 32 | +1%         | +50MB       | 0.02%/MB     |
| 32 → 64 | +1.5%       | +180MB      | 0.008%/MB    |
...
Valid = { (M, ef_construction) |
          build_time(M, ef_construction) ≤ T_max AND
          memory(M, ef_construction) ≤ M_max AND
          recall(M, ef_construction) ≥ R_min }
```

**Step 3: Select Optimal Configuration**
```
Optimal = argmax_{(M, ef_construction) ∈ Valid} recall(M, ef_construction)
```
...
- (M=16, ef_construction=300): build=135s, mem=120MB, recall=0.96 ✅
- (M=24, ef_construction=200): build=135s, mem=170MB, recall=0.96 ✅
- (M=16, ef_construction=400): build=180s, mem=122MB, recall=0.97 ✅
...
| ---------------------| ---------| ---------| ------|
| Vector Size          | 6 KB     | 12 KB    | 2×    |
| Index Memory (100K)  | 120 MB   | 340 MB   | 2.8×  |
| Build Time (100K)    | 90s      | 270s     | 3×    |
| Search Latency (P95) | 35 ms    | 60 ms    | 1.7×  |
| Recall@10 (default)  | 0.94     | 0.95     | +1%   |

**Key Insight:** Doubling dimensions requires ~3× memory and build time for similar recall.

### 8.4 Migration Strategy (1536 → 3072)

**When to Upgrade:**
- New embedding model requires 3072 dimensions
- Recall requirements increase (≥ 0.98)
- Memory budget allows 3× increase
...
**Use Case:** Critical search applications (≥ 0.98 recall required)
...
| Memory Usage         | < 2× vector data | > 3× vector data |
...
- [ ] Recall@10 ≥ target (0.95 for production)
- [ ] Recall@100 ≥ target (0.98 for production)
- [ ] Search latency (P95) < 100 ms
- [ ] Build time < 5 min per 100K vectors
- [ ] Memory usage < 2× vector data size
