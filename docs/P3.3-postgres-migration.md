# P3.3 — PostgreSQL to ClawFS SQLite Migration Guide
...
- **In Scope**:
  - PostgreSQL schema → SQLite schema migration
  - pgvector vectors → HNSW index migration
...
#### vector(1536) → BLOB (Float32 Array)
...
Phase 2: Data Export (PostgreSQL)
  ...
  ├─ Export vectors table
  ├─ Export metadata table
  └─ Generate checksums

Phase 3: Data Import (SQLite)
  ├─ Import documents to FTS5
...
| row |
    ...
    println!("✓ Document count verified: {}", pg_doc_count);
...
| row |
    ...
    println!("✓ Vector count verified: {}", pg_vec_count);
...
| row |
    ...
    println!("✓ Vector dimension verified: {}", pg_dim);
...
| -------------: | ----------: | ------------: | ----------: | ----------------------: |
| **P1-P2**     | Active     | ✅ Primary    | ❌ Not used | IronClaw compatibility |
| **P3 (E-02)** | Migration  | ✅ Dual-write | ✅ Primary  | Migration in progress  |
| **P3 (E-03)** | Transition | ⚠️ Read-only | ✅ Primary  | Code refactoring       |
| **P4**        | Deprecated | ❌ Removed    | ✅ Primary  | PostgreSQL removed     |
