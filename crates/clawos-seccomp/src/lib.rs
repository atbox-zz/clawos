...
pub fn apply_filter() -> Result<()> {
    ...
    for name in WHITELIST {
        let sc =
            ScmpSyscall::from_name(name).with_context(|| format!("Unknown syscall: {name}"))?;
        filter
    ...
    for name in EXTRA_SYSCALLS {
        let sc = ScmpSyscall::from_name(name)
            .with_context(|| format!("Unknown extra syscall: {name}"))?;
        filter
            .add_rule(ScmpAction::Allow, sc)
            .with_context(|| format!("Failed to add extra rule for: {name}"))?;
    }
...
// FIX BUG-5: recvfrom, sendto, getsockopt were listed in both WHITELIST and
// EXTRA_SYSCALLS — libseccomp add_rule was called twice for each.
// Benign today but risks Err on stricter libseccomp versions. Deduplicated:
// the three syscalls remain in WHITELIST; EXTRA_SYSCALLS now contains only
// entries that are genuinely absent from WHITELIST.
const EXTRA_SYSCALLS: &[&str] = &[
    "mkdir", "mkdirat", "unlink", "unlinkat", "listen", "accept", "accept4",
    "chmod", "fchmod", "fchmodat", "execve", "dup", "dup2", "dup3",
    "pidfd_open",
];
...
const WHITELIST: &[&str] = &[
    ...
    // File IO
    "read", "write", "pread64", "pwrite64", "readv", "writev", "open",
    "openat", "openat2", "close", "close_range", "stat", "fstat", "lstat",
    "newfstatat", "statx", "lseek", "fcntl", "ioctl", "fsync", "fdatasync", "getcwd",
    // Process control
    "exit", "exit_group", "getpid", "getppid", "gettid", "getuid", "getgid", "geteuid",
    "getegid", "futex", "set_robust_list", "get_robust_list", "clone", "clone3", "wait4",
    "waitid", "prctl", "arch_prctl", "set_tid_address",
    // Network
    "socket", "connect", "bind", "recv", "send", "recvfrom", "sendto", "recvmsg",
    "sendmsg", "setsockopt", "getsockopt", "getsockname", "getpeername", "shutdown",
    ...
    "kill", "tgkill",
    // Time
    "clock_gettime", "clock_nanosleep", "clock_getres", "gettimeofday", "nanosleep", "timer_create",
    "timer_settime", "timer_gettime", "timer_delete",
    // IPC
    "pipe2",
    "eventfd2",
    // Misc
    "getrandom", "uname", "getrlimit", "setrlimit", "prlimit64", "sysinfo", "rseq",
];
