...
impl ToolRegistry {
    ...
    pub async fn scan(&mut self) -> Result<()> {
        ...
        // Release build guard: reject any stub:// paths that may have been
        // injected by register_stub() before scan() was called.
        #[cfg(not(debug_assertions))]
        {
            let stub_tools: Vec<&str> = self
                .tools
                .values()
                .filter(|m| m.wasm_path.starts_with("stub://"))
                .map(|m| m.name.as_str())
                .collect();
            if !stub_tools.is_empty() {
                anyhow::bail!(
                    "Release build: stub tools detected in registry: {:?}. \
                     Deploy real WASM binaries to {}/*/tool.wasm.",
                    stub_tools,
                    self.tools_dir
                );
            }
    ...
    /// Register a built-in stub tool.
    ///
    /// Stubs are only permitted in debug builds — they represent tools that haven't
    /// had their WASM binary deployed yet.  In release builds this method panics to
    /// prevent accidentally shipping stub tools to production.
    pub fn register_stub(&mut self, name: &str, description: &str, caps: Vec<Capability>) {
        // FIX BUG-3: previously the cfg guard was placed only on the `let manifest`
        // binding, so in a release build the binding was absent but the
        // `self.tools.insert(…, manifest)` line still referenced it (compile error).
        // The panic also ran unconditionally in debug after the insert.
        // Corrected: the entire stub path is gated; release panics before inserting.
        #[cfg(not(debug_assertions))]
        {
            panic!(
                "register_stub('{}') called in a release build. \
                 Deploy the real WASM binary to {:?}/{}/tool.wasm and use \
                 scan() to load it instead.",
                name, self.tools_dir, name
            );
        }
        ...
        #[cfg(debug_assertions)]
        {
            ...
            warn!(
                tool = name,
                "Stub tool registered — dev/test only, not for production"
            );
