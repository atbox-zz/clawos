...
impl Router {
    ...
    fn try_slash_command(&self, s: &str) -> Option<Intent> {
        ...
        match parts[0] {
            "/jobs" => Some(Intent::Admin {
                command: AdminCommand::ListJobs,
            }),
            "/cancel" => parts.get(1).map(|id| Intent::Admin {
                command: AdminCommand::CancelJob {
                    job_id: id.to_string(),
                },
            }),
            "/memory" => Some(Intent::Admin {
                command: AdminCommand::ShowMemory {
                    query: parts.get(1).map(|s| s.to_string()),
                },
            }),
            "/status" => Some(Intent::Admin {
                command: AdminCommand::AgentStatus,
            }),
            "/gate" => parts.get(1).map(|phase| Intent::Admin {
                command: AdminCommand::GateCheck {
    ...
    fn try_json_tool_call(&self, s: &str) -> Option<Intent> {
        ...
        // Validate tool exists before returning ToolCall intent (avoids wasted round-trip).
        if !self.known_tools.iter().any(|t| t == &tool) {
            warn!(tool = %tool, "JSON tool call references unknown tool — falling through to Query");
            return None;
        }
...
mod tests {
    ...
    #[test]
    fn unknown_json_tool_falls_back_to_query() {
        let r = router();
        let raw = r#"{"tool":"nonexistent-tool","args":{}}"#;
        // Should NOT be classified as ToolCall — falls through to Query
        assert!(matches!(r.classify(raw), Intent::Query { .. }));
    }
