    ...
    F: FnOnce() -> i32 + Send + 'static,
{
    ...
    let child_pid = unsafe {
        nix::sched::clone(
            Box::new(move || f_opt.take().unwrap()() as isize),
            &mut stack,
            flags,
...
fn setup_uid_map(child: nix::unistd::Pid, host_uid: u32, host_gid: u32) -> Result<()> {
    ...
    fs::write(format!("/proc/{pid}/setgroups"), "deny").context("Failed to write setgroups")?;
...
pub fn do_pivot_root(new_root: &str) -> Result<()> {
    ...
    nix::unistd::pivot_root(new_root, put_old.as_str()).context("pivot_root failed")?;

    nix::unistd::chdir("/").context("chdir / after pivot_root failed")?;
...
pub fn mount_minimal_fs() -> Result<()> {
    ...
    let nosuid_nodev_noexec = nix::mount::MsFlags::MS_NOSUID
