    ...
    pub async fn freeze(&self, entry: VaultEntry) -> Result<()> {
        ...
        if record_path.exists() {
            ...
            if existing.sha256 == entry.sha256 {
                ...
                anyhow::bail!(
                    "E002: SPEC_HASH_MISMATCH — vault entry {} is frozen and cannot be modified. \
                    Requires dual-agent review (Security + Core Dev).",
                    entry.spec_id
                );
            }
        }
        ...
        tokio::fs::write(&record_path, json)
            .await
...
if entry.sha256 != actual_sha256 {
        ...
        let expected_prefix = if entry.sha256.len() >= 16 {
            &entry.sha256[..16]
        } else {
            &entry.sha256
        };
        let actual_prefix = if actual_sha256.len() >= 16 {
            &actual_sha256[..16]
        } else {
            &actual_sha256
        };
        anyhow::bail!(
            "E002: SPEC_HASH_MISMATCH for '{}' — expected {}, got {}",
            spec_id, expected_prefix, actual_prefix
        );
    }
...
mod tests {
    ...
    async fn write_once_same_hash_is_noop() {
        ...
        let entry = VaultEntry {
            spec_id: "P1.1".into(),
            path: "/x".into(),
            sha256: "aabb".repeat(16),
            frozen_at: 0,
            signed_by: "WASM Agent".into(),
            phase: "P1".into(),
        };
